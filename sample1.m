...
N_blk=15; % The number of blocks used in the measurement phase
...
itv_Em=2; % The interval between two adjacent energy measurements
suffix=datestr(now,'_yymmdd_HHMMSS'); % time stamp for the saved *.mat filename. Can be changed to any desired string 

%% invoke the main function
[E_ave,E_err,ss_ave, ss_err, cc_ave, cc_err, savedFile]=CPMC_Lab(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em, i, j, suffix);
...
figure;
plot ((1:N_blk)*N_blksteps*deltau,E);
xlabel ('tau');
ylabel ('E');
