function [phi, w, O, O_prov, E, W] = stepwlk_multi(phi, N_wlk, w, O, O_prov, E, W, H_k, Proj_k_half, flag_mea, Phi_T, w_T, N_up, N_par, N_sites, N_det, U, fac_norm, aux_fld)
...
% "CPMC-Lab: A Matlab Package for Constrained Path Monte Carlo Calculations" Comput. Phys. Commun. (2014)
    ...
    for i_wlk=1:N_wlk
        ...
        if w(i_wlk)>0
            ...
            [Phi, w(i_wlk), O(i_wlk), O_prov(i_wlk,:), invO_matrix_up, invO_matrix_dn]=halfK_multi(Phi, w(i_wlk), O(i_wlk), O_prov(i_wlk,:), Proj_k_half, w_T, Phi_T, N_up, N_par, N_det);
            if w(i_wlk)>0
                % propagate each lattice site of a walker by the potential term:
                for j_site=1:N_sites
                    if w(i_wlk)>0
                        [Phi(j_site,:), O(i_wlk), O_prov(i_wlk,:), w(i_wlk), invO_matrix_up, invO_matrix_dn]=V_multi(Phi(j_site,:), Phi_T(j_site,:,:), N_up, N_par, N_det, O(i_wlk), O_prov(i_wlk,:), w(i_wlk), w_T, invO_matrix_up, invO_matrix_dn, aux_fld);
                    end
                end
            end
            if w(i_wlk)>0
                ...
                if w(i_wlk)>0
                    ...
                    if flag_mea==1
                        [e(i_wlk)]=measure_multi(H_k, Phi(:,:), Phi_T, w_T, O(i_wlk), O_prov(i_wlk,:), invO_matrix_up, invO_matrix_dn, N_up, N_par, N_sites, N_det, U);
                    end
                end
            end
        end
