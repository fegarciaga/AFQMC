function [E_ave,E_err, ss_ave, ss_err, cc_ave, cc_err, savedFileName]=CPMC_Lab(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,tx2,ty2,tz2,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em, i, j,suffix)
...
%% Initialization
tic; % start the  timer
initialization3; % initialize internal constants, form the trial wave function and assemble the initial population of walkers
format long;
...
ss_blk=zeros(N_blk,1);
cc_blk=zeros(N_blk,1);
%% Equilibration phase
for i_blk=1:N_eqblk
    for j_step=1:N_blksteps
        [Phi, w, O, E, W, SS, CC] = stepwlk(Phi, N_wlk, N_sites, w, O, E, W, SS, CC, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        [Phi, w, O, E_blk(i_blk), W_blk(i_blk), ss_blk(i_blk), cc_blk(i_blk)] = stepwlk(Phi, N_wlk, N_sites, w, O, E_blk(i_blk), W_blk(i_blk), ss_blk(i_blk), cc_blk(i_blk), H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld, i, j);
        if mod(j_step,itv_modsvd)==0
            [Phi, O] = stblz(Phi, N_wlk, O, N_up, N_par); % re-orthonormalize the walkers
        end
        if mod(j_step,itv_pc)==0
    ...
    ss_blk(i_blk)=ss_blk(i_blk)/W_blk(i_blk);
    cc_blk(i_blk)=cc_blk(i_blk)/W_blk(i_blk);
