function [phi, w, O, E, W, SS, CC] = stepwlk(phi, N_wlk, N_sites, w, O, E, W, SS, CC, H_k, Proj_k_half, flag_mea, Phi_T, N_up, N_par, U, fac_norm, aux_fld, i, j)
...
if flag_mea==1
    for i_wlk=1:N_wlk
        if w(i_wlk)>0
            E=E+e(i_wlk)*w(i_wlk);
            SS=SS+ss_corr(i_wlk)*w(i_wlk);
            CC=CC+cc_corr(i_wlk)*w(i_wlk);
            W=W+w(i_wlk);
        end
    end
end
